...
class ProgressBar {
 private:
  ...
  const long unsigned int total_ticks;
 ...
 public:
  ProgressBar(long unsigned int total, unsigned int width, char complete, char incomplete) :
	  total_ticks {total}, bar_width {width}, complete_char {complete}, incomplete_char {incomplete} {}

  ProgressBar(long unsigned int total, unsigned int width) : total_ticks {total}, bar_width {width} {}

  long unsigned int operator++() { return ++ticks; }
  long unsigned int operator+=(const unsigned int n) { ticks+=n; return ticks;}
